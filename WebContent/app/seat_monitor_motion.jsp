...
<body>
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <i class="fa fa-bar-chart-o fa-fw"></i> Motion sensor real time data
                        </div>
                        <!-- /.panel-heading -->
	...
	<script src="jslib/appcommon.js"></script>
   	...
   	<script>
   	var OCCUPENCY_THRESHOLD = <%=ApplicationConfigFactory.getLongProperty(ApplicationConfigFactory.MOTION_BASED_OCCUPENCY_TH, 20L)%>;;
   	var MOTION_DATA_HISTORY_LIMIT = <%=ApplicationConfigFactory.getLongProperty(ApplicationConfigFactory.MOTION_DATA_HISTORY_LIMIT, 1200000L)%>;
    ...
    $(document).ready(function () {
    		...
    		$.getJSON( "retrieveMotionSensorData.wss?rand="+Math.random()+"&deviceId="+$("#deviceId").val(), function( data ) {
  				if(data!=null)
  				{
    ...
    function setupAll()
    {
    	...
    	//Setup the graph
    	if(LAST_ONE_HOUR_DATA.length> 20)
    	{
    		var initialData = [];
    		//for()
    		exisitingGrpah = new RunningGraph("admin_home_running_graph","Motion sensor data from device id "+deviceId,"Motion","Motion sensor data");	
    	}
    	else
    	{
    		exisitingGrpah = new RunningGraph("admin_home_running_graph","Motion sensor data from device id "+deviceId,"Motion","Motion sensor data");	
    	...
    	//Setup the gauge
    	meterGauge = new  MeterGauge("occupency_ind_ph","Motion count meter",20,10,0);
    	//Do an inital analysis
    	analyze();
		...
		{	
			mqttClient = new MQTTClient(iotConfig.orgId,iotConfig.username,iotConfig.password,topic,function(message){
				try{
					...
					var sensortype = "motion";
					var x = (new Date()).getTime();
					var y = parseFloat(payload["d"][sensortype]);
    ...
    /* Function to analyze the motion data
   	*/
   	function analyze()
   	{
   		...
   		{
   			var dataPoint = LAST_ONE_HOUR_DATA[index];
   			motionCount = motionCount+ dataPoint.motion;
   			if(motionCount>=OCCUPENCY_THRESHOLD*2)
   			{
   				break;
   			}
   		}
   		console.log("Motion count" + motionCount);
   		meterGauge.updateReading(motionCount);
   		if(motionCount>=OCCUPENCY_THRESHOLD)
		{
   			//set occupency to OCCUPIED
   			$("#occupied").bootstrapSwitch('state', true, true);
		}
   		else
   		{
   			//set occupency to EMPTY
   			//setOccupency("Not Occupied",motionCount);
   			$("#occupied").bootstrapSwitch('state', false, true);
   		}
   		//Remove 1 hour old data
   		var currTime = (new Date()).getTime();
   		
   		if(currTime - LAST_ONE_HOUR_DATA[0].ts >= MOTION_DATA_HISTORY_LIMIT || LAST_ONE_HOUR_DATA.length>240 )
   		{
